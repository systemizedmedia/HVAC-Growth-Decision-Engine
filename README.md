...
The tool now calculates MER automatically from the rows entered:

```text
Entered Rows MER = Total Entered Revenue / Total Entered Spend
```

This is best interpreted as **paid media MER** or **entered-channel MER** if the user is only entering paid channels.

Channel-level tables also show:
