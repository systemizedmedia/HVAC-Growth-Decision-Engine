...
function analyze() {
  ...
  renderAllocation(rows, summary);
  renderSpecificScenarios(rows, summary);
...
function renderDiagnosticCards(summary, rows) {
  ...
  const cards = [
    ...
    {
      label: "Best Channel Rev/Spend",
      value: bestByMer ? getGroupLabel(bestByMer, level) : "N/A",
      detail: bestByMer ? `${formatRatio(bestByMer.mer)} revenue per $1 spent` : "Revenue is required for revenue/spend."
    },
    {
    ...
    {
      label: "Entered Rows MER",
      value: formatRatio(summary.mer),
      detail: "Calculated automatically as total entered revenue ÷ total entered spend."
    },
    {
      label: "Rows Entered",
...
function renderAllocation(rows, summary) {
  const level = $("rankLevel").value;
  const objective = $("objectiveSelect").value;
  ...
  const bestProjectedMer = calculateProjectedMer(summary, bestProjected.revenue, scenarioBudget);
  ...
  const proportionalProjectedMer = calculateProjectedMer(summary, propTotals.revenue, scenarioBudget);
  ...
  $("allocationOutput").innerHTML = `
    <div class="allocation-card">
      ...
      <ul>
        ...
        <li>Projected total MER after extra budget: <strong>${formatRatio(bestProjectedMer)}</strong></li>
      </ul>
      <p class="note">
        Current entered rows MER: ${formatRatio(summary.mer)}. Projected MER includes current revenue plus projected added revenue, divided by current spend plus the extra budget.
      </p>
    </div>
    ...
    <div class="allocation-card">
      ...
      <p class="note">
        Projected total MER after proportional allocation: <strong>${formatRatio(proportionalProjectedMer)}</strong>.
        Current entered rows MER: ${formatRatio(summary.mer)}.
      </p>
    </div>
  `;
}

function renderSpecificScenarios(rows, summary) {
  const level = $("rankLevel").value;
  const objective = $("objectiveSelect").value;
  const scenarioBudget = toNumber($("budgetInput").value);
  const keys = level === "campaign" ? ["channel", "campaign"] : ["channel"];
  ...
  const ranked = groups.map(item => {
    ...
    ...item,
    projectedTotalMer: calculateProjectedMer(summary, item.projection.revenue, scenarioBudget)
  ...
  const body = ranked.map((item, index) => `
    <tr>
      ...
      <td>${formatRatio(item.projectedTotalMer)}</td>
    </tr>
  `).join("");
...
function calculateProjectedMer(summary, projectedAddedRevenue, extraBudget) {
  return safeDiv(summary.revenue + projectedAddedRevenue, summary.spend + extraBudget);
}
...
["timeframeName", "budgetInput", "rankLevel", "objectiveSelect"].forEach(id => {
